...
const PantryScanningService = {
    async identifyFoodItemsInPantry(imageFile) {
        try {
            ...
            const response = await openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: "system",
                        content: "You are an assistant that helps identify food items in pantry photos. Your task is to analyze the image and list all visible food items, ingredients, or groceries that you can identify. Return a JSON object with a single key 'foodItems' containing an array of food item strings in lowercase, with no additional information. Example format: { \"foodItems\": [\"pasta\", \"canned beans\", \"rice\", \"cereal\"] }"
                    },
                    {
                        role: "user",
                        content: [
                            { type: "text", text: "Please identify all food items visible in this pantry photo." },
                            {
                                type: "image_url",
                                image_url: {
            ...
            console.error('Error identifying food items in pantry:', error);
            throw error;
        }
    }
};

export { PantryScanningService };