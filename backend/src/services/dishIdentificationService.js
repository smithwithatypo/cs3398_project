...
const DishIdentificationService = {
    async identifyDishFromImage(imageFile) {
        try {
            ...
            let formattedRecipe = response.choices[0].message.content
                .replace(/^####\s*(.+)$/gm, '<strong>$1</strong><br>')  // Bold level 4 headings with line break
                .replace(/^###\s*(.+)$/gm, '## $1 ')   // Convert level 3 
